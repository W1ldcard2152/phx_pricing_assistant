...
• "This model had a recall for fuel pump issues, so there are many refurbished/updated versions available at premium prices. Focus on standard used parts."""
    ...
    def _analyze_prices_with_ai(self, raw_items: List[Dict], part_name: str, minimum_price: float = 0) -> Dict[str, float]:
        ...
        try:
            ...
            # Comprehensive AI prompt for thorough analysis
            prompt = f"""You are an expert automotive parts price analyst. Analyze these {len(prices)} eBay listings for "{part_name}" and provide pricing tiers for used/salvage parts.

**LISTING DATA:**
{data_text}

**ANALYSIS INSTRUCTIONS:**
1. Filter OUT: new parts, aftermarket parts, accessories (switches, sensors, small components), wrong vehicle applications, damaged cores, obvious outliers
2. Focus ON: used OEM parts, salvage yard parts, refurbished/rebuilt parts suitable for this vehicle
3. Calculate pricing tiers: Budget (low), Standard (mid-range), Premium (high-quality used)
4. Assess data quality and confidence level based on sample size, consistency, and relevance

**CONFIDENCE LEVELS:**
- dark_green: Excellent data (50+ relevant items, consistent pricing)
- light_green: Good data (20-49 relevant items, mostly consistent)  
- yellow: Moderate data (10-19 relevant items, some inconsistency)
- orange: Poor data (5-9 relevant items, high variability)
- red: Very poor data (<5 relevant items, mostly inappropriate listings)
...
{{
  ...
  "reasoning": "Detailed analysis explaining filtering decisions and pricing rationale",
  "confidence_rating": "yellow",
  "confidence_explanation": "Specific reason for confidence level based on data quality"
}}"""
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    self.results_text.insert(tk.END, f"OpenAI API call successful\n")
                    self.root.update()
                    break
